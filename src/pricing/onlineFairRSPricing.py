...
def calculate_similarity(df):
    ...
    np.savetxt("../data/similarity_10_9to13_3km_default.csv", similarity, delimiter=",")
    # return similarity
...
def fair(Distribution,gamma,omega,price,cost,similarity,K,a):
    ...
    constraints.append({'type': 'ineq', 'fun': lambda d: (K*similarity-(np.tile(d,m).reshape(m,m).T-np.tile(d,m).reshape(m,m))).flatten()}) # fairness constraints

    # s = time.time()
    res = minimize(fun=ProfitOBJ, x0=d0, args=(Distribution,gamma,omega,price,cost), method='SLSQP',constraints=constraints,options={'maxiter':100})
...
def ProfitOnly(Distribution,gamma,omega,price,cost,a):
    ...
    # s = time.time()
    res = minimize(fun=ProfitOBJ, x0=d0, args=(Distribution,gamma,omega,price,cost), method='SLSQP',constraints=constraints,options={'maxiter':200})
    # print('Time: ',time.time()-s)
    # print('Opt value: ',res.fun)
    # print('Success: ', res.success)
    # print('Iteration: ',res.nit)
...
def learning(order,gamma,Omega,omega,a,threshold):
    ...
    while t < T:
        ...
        temp = np.tile(Xi,len(Omega)).reshape(len(Omega),len(Omega)).T - np.tile(Xi,len(Omega)).reshape(len(Omega),len(Omega))
...
def onlineFairRSPricing(OMEGA, omega, a, K, eta_peak, eta_off, sigma, threshold=1,Transition=600):
    '''
    ...
    '''
    # others = read_others()
    others = pd.read_csv("../data/others_10_9to13_3km.csv")
    others['gamma'] = others.apply(lambda x: sigma* np.random.randn() + eta_off if x['tau'] == 3 else sigma* np.random.randn() + eta_peak, axis=1)
    ...
    df = others[['o_lat', 'o_lng', 'd_lat', 'd_lng', 'tau']]
    ...
    similarity = pd.read_csv("../data/similarity_10_9to13_3km_default.csv",header=None).values
    ...
    others.to_csv('../data/others_10_9to13_3km_default.csv',index=False)

    discount = others[['o','d','tau','odt','gamma','ProfitOnly','FIXED','OnFair']]
    all_riders = pd.merge(all_riders,discount,on=['o','d','tau'])

    all_riders['OnFair_all'] = all_riders['OnFair']

    all_riders.loc[:t+Transition-1,'OnFair'] = [a]*(t+Transition)
    all_riders['OnFair_status'] = all_riders.apply(lambda x: 1 if x['rnd'] < x['gamma'] - x['OnFair'] * omega else 0, axis=1)
    all_riders.loc[:t-1,'OnFair_status'] = order_status
    all_riders['OnFair_all_status'] = all_riders.apply(lambda x: 1 if x['rnd'] < x['gamma'] - x['OnFair_all'] * omega else 0, axis=1)
    all_riders['ProfitOnly_status'] = all_riders.apply(lambda x: 1 if x['rnd'] < x['gamma'] - x['ProfitOnly'] * omega else 0, axis=1)
    all_riders['FIXED_status'] = all_riders.apply(lambda x: 1 if x['rnd'] < x['gamma'] - x['FIXED'] * omega else 0, axis=1)

    all_riders.to_csv('../data/haikou_10_16_3km_default.csv',index=False)  
...
def fairness_metrics(algo,K,order_file='../data/haikou_10_16_3km_default.csv'):
    '''
    ...
    all_riders = riders[riders[algo+'_status']==1].reset_index()
    m = len(all_riders)
    ...
    all_riders[algo+'_ratio2'] = all_riders.apply(lambda x:all_riders[all_riders.index <= x['rider_index']][algo+'_vio2_status'].sum()/all_riders[all_riders.index <= x['rider_index']][algo+'_status'].sum(), axis=1)
